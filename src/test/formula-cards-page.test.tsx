...
describe('formula cards page', () => {
  ...
  it('renders the full formula registry and one selected card for service mode', () => {
    ...
    const rectangularInsetRow = screen.getByText('PRM-009').closest('tr')
    expect(rectangularInsetRow).not.toBeNull()
    fireEvent.click(rectangularInsetRow!)

    expect(rectangularInsetRow).toHaveAttribute('aria-selected', 'true')
    expect(within(details!).getAllByText('PRM-009').length).toBeGreaterThan(0)
    expect(within(details!).getByText('Врезка прямоугольная')).toBeInTheDocument()
    expect(within(details!).getByText('Не смешивать PRM-009 с PRM-010, PRM-005, KMB-004 или KRG-008.')).toBeInTheDocument()
    expect(
      within(details!).getByText('Не вычитать отверстие основного ствола, если PRM-009 является отдельной врезкой без подтверждённого ствола.'),
    ).toBeInTheDocument()
    expect(within(details!).getByText('Sчистовая = 2 × (W + H) × L / 1_000_000')).toBeInTheDocument()
    expect(within(details!).getByText('Sчистовая = Sпатрубок + Sворотник')).toBeInTheDocument()
    expect(
      within(details!).getByText('b — высота воротника/борта в мм; b нельзя фиксировать без подтверждения CAMduct/методички.'),
    ).toBeInTheDocument()
    expect(within(details!).getByText('Не ставить “Проверена” только из-за СПВА.')).toBeInTheDocument()
    expect(within(details!).queryByText(/^Проверена$/)).not.toBeInTheDocument()
    expect(within(details!).getByText('Альтернативные источники / справочные формулы')).toBeInTheDocument()
    expect(within(details!).getByText('S = 2 × (Ш + Г) × Д × n')).toBeInTheDocument()
    expect(within(details!).getByText(/Внешний вариант с круглым вырезом через D; требует сопоставления с CAMduct/)).toBeInTheDocument()

    const rectangularSaddleRow = screen.getByText('PRM-010').closest('tr')
    expect(rectangularSaddleRow).not.toBeNull()
    fireEvent.click(rectangularSaddleRow!)

    expect(rectangularSaddleRow).toHaveAttribute('aria-selected', 'true')
    expect(within(details!).getAllByText('PRM-010').length).toBeGreaterThan(0)
    expect(within(details!).getByText('Прямоугольный воздуховод с прямоугольной врезкой')).toBeInTheDocument()
    expect(within(details!).getByText('Не смешивать PRM-010 с PRM-009, PRM-005, KMB-004 или KRG-009.')).toBeInTheDocument()
    expect(within(details!).getByText('PRM-010 не является отдельной PRM-009 без основного ствола.')).toBeInTheDocument()
    expect(within(details!).getByText('Sчистовая = Sствол + Sврезка − Sотверстие_ствол')).toBeInTheDocument()
    expect(within(details!).getByText('Sотверстие_ствол = W2 × H2')).toBeInTheDocument()
    expect(within(details!).getByText('Если PRM-010 использует отдельную PRM-009 как компонент: Sчистовая = Sствол + S_PRM-009 − Sотверстие_ствол')).toBeInTheDocument()
    expect(within(details!).getByText('По компонентам: конструкция рассматривается как PRM-001 + PRM-009 + отверстие в основном стволе.')).toBeInTheDocument()
    expect(within(details!).getByText('Не ставить “Проверена” только из-за СПВА.')).toBeInTheDocument()
    expect(within(details!).queryByText(/^Проверена$/)).not.toBeInTheDocument()
    expect(within(details!).getByText('Альтернативные источники / справочные формулы')).toBeInTheDocument()
    expect(within(details!).getByText('S = 2 × (Ш + Г) × Д × n')).toBeInTheDocument()
    ...
    const rectangularDamperRow = screen.getByText('PRM-013').closest('tr')
    expect(rectangularDamperRow).not.toBeNull()
    fireEvent.click(rectangularDamperRow!)

    expect(rectangularDamperRow).toHaveAttribute('aria-selected', 'true')
    expect(within(details!).getAllByText('PRM-013').length).toBeGreaterThan(0)
    expect(within(details!).getByText('Дроссель прямоугольный')).toBeInTheDocument()
    expect(within(details!).getByText('Не смешивать PRM-013 с PRM-001, PRM-008, PRM-011, KRG-013 или KRG-017.')).toBeInTheDocument()
    expect(within(details!).getByText('PRM-013 не должен считаться только как PRM-001 без внутренней заслонки.')).toBeInTheDocument()
    expect(within(details!).getByText('Sкорпус = 2 × (W + H) × L / 1_000_000')).toBeInTheDocument()
    expect(within(details!).getByText('Условная компонентная формула: Sчистовая = Sкорпус + Sзаслонка')).toBeInTheDocument()
    expect(within(details!).getByText('Если есть несколько лопастей: Sчистовая = Sкорпус + n × Sлопасть')).toBeInTheDocument()
    expect(within(details!).getByText('Не ставить “Проверена” только из-за формулы корпуса PRM-001.')).toBeInTheDocument()
    expect(within(details!).queryByText(/^Проверена$/)).not.toBeInTheDocument()
    expect(within(details!).getByText('СПВА не подтверждает внутреннюю компонентную структуру PRM-013; прямой СПВА-источник для прямоугольного дросселя не добавлен.')).toBeInTheDocument()
    expect(within(details!).queryByText('Альтернативные источники / справочные формулы')).not.toBeInTheDocument()

    const roundToRectTransitionRow = screen.getByText('KMB-001').closest('tr')
    expect(roundToRectTransitionRow).not.toBeNull()
    fireEvent.click(roundToRectTransitionRow!)

    expect(roundToRectTransitionRow).toHaveAttribute('aria-selected', 'true')
    expect(within(details!).getAllByText('KMB-001').length).toBeGreaterThan(0)
    expect(within(details!).getByText('Переход прямоугольник → круг')).toBeInTheDocument()
    expect(within(details!).getByText('KMB-001 — переход прямоугольник → круг: единственная CAMduct-модель с прямоугольным входом C1 и круглым выходом C2.')).toBeInTheDocument()
    expect(within(details!).getByText('C1 / вход = прямоугольный торец W × H.')).toBeInTheDocument()
    expect(within(details!).getByText('C2 / выход = круглый торец D.')).toBeInTheDocument()
    expect(within(details!).getByText('Не смешивать KMB-001 с KRG-003, KRG-004, PRM-004, KRG-001 или PRM-001.')).toBeInTheDocument()
    expect(within(details!).getByText('KMB-001 не должен считаться простой суммой круглого и прямоугольного воздуховода.')).toBeInTheDocument()
    expect(within(details!).getByText('KMB-001 не имеет рабочего варианта “круг → прямоугольник” в текущей CAMduct-модели.')).toBeInTheDocument()
    expect(within(details!).queryByText('KMB-001 — переход круг-прямоугольник: комбинированное фасонное изделие с круглым и прямоугольным торцами.')).not.toBeInTheDocument()
    expect(within(details!).getByText('Для центрального перехода без смещения: Sчистовая ≈ ((π × D + 2 × (W + H)) / 2) × L / 1_000_000')).toBeInTheDocument()
    expect(within(details!).getByText('Если CAMduct считает по панелям: Sчистовая = Σ Sпанель_i')).toBeInTheDocument()
    expect(within(details!).getByText('Средний периметр — не проверенная CAMduct-формула, а инженерная оценка.')).toBeInTheDocument()
    expect(within(details!).getByText('Не ставить “Проверена” только из-за СПВА.')).toBeInTheDocument()
    expect(within(details!).queryByText(/^Проверена$/)).not.toBeInTheDocument()
    expect(within(details!).getByText('Альтернативные источники / справочные формулы')).toBeInTheDocument()
    expect(within(details!).getByText('S = [((D/2 + Ш) × Д + (D/2 + В) × Д) × 1.15] × n')).toBeInTheDocument()
    expect(within(details!).getByText(/для KMB-001 фиксируется направление C1 прямоугольник → C2 круг/)).toBeInTheDocument()
    expect(within(details!).getByText(/В более понятной записи: S = \[\(\(D\/2 \+ W\) × L \+ \(D\/2 \+ H\) × L\) × 1\.15\] × n/)).toBeInTheDocument()

    const rectangularSaddleSeatRow = screen.getByText('KMB-002').closest('tr')
    expect(rectangularSaddleSeatRow).not.toBeNull()
    fireEvent.click(rectangularSaddleSeatRow!)

    expect(rectangularSaddleSeatRow).toHaveAttribute('aria-selected', 'true')
    expect(within(details!).getAllByText('KMB-002').length).toBeGreaterThan(0)
    expect(within(details!).getByText('Седло с прямоугольной врезкой')).toBeInTheDocument()
    expect(within(details!).getByText('Не смешивать KMB-002 с PRM-009, PRM-010, KMB-004, KRG-009 или KRG-008.')).toBeInTheDocument()
    expect(within(details!).getByText('KMB-002 не должен автоматически считаться PRM-009: у KMB-002 есть седловая подрезка / сопряжение с круглой поверхностью.')).toBeInTheDocument()
    expect(within(details!).getByText('Если KMB-002 — прямоугольный патрубок с седловой подрезкой без воротника: Sчистовая ≈ Sпатрубок_седловой')).toBeInTheDocument()
    expect(within(details!).getByText('Sсторона_i = ширина_стороны_i × Lэфф_i')).toBeInTheDocument()
    expect(within(details!).getByText('Не выводить Lэфф_i без подтверждения CAMduct.')).toBeInTheDocument()
    expect(within(details!).getByText('Если в составе есть основной круглый ствол: Sчистовая = Sствол_круглый + Sврезка_прямоугольная − Sотверстие_ствол + Sворотник/седловая_зона')).toBeInTheDocument()
    expect(within(details!).getByText('Не ставить “Проверена” только из-за СПВА.')).toBeInTheDocument()
    expect(within(details!).queryByText(/^Проверена$/)).not.toBeInTheDocument()
    expect(within(details!).getByText('Альтернативные источники / справочные формулы')).toBeInTheDocument()
    expect(within(details!).getByText('S = 2 × (Ш + Г) × Д × n')).toBeInTheDocument()
    expect(within(details!).getByText('S = [(2 × (Ш + Г) × Д + Ш × Г − π × (D / 2)²) × 1.1] × n')).toBeInTheDocument()
    expect(within(details!).getByText(/только внешний ориентир, ближе к отдельной PRM-009/)).toBeInTheDocument()
    expect(within(details!).getByText(/Требует сопоставления с CAMduct и типом седла/)).toBeInTheDocument()

    const roundRectTeeRow = screen.getByText('KMB-003').closest('tr')
    expect(roundRectTeeRow).not.toBeNull()
    fireEvent.click(roundRectTeeRow!)

    expect(roundRectTeeRow).toHaveAttribute('aria-selected', 'true')
    expect(within(details!).getAllByText('KMB-003').length).toBeGreaterThan(0)
    expect(within(details!).getByText('Тройник круг-прямоугольник')).toBeInTheDocument()
    expect(within(details!).getByText('Не смешивать KMB-003 с KRG-005, PRM-005, PRM-009, KMB-002 или KMB-004.')).toBeInTheDocument()
    expect(within(details!).getByText('KMB-003 не должен автоматически считаться KRG-005, потому что ветка прямоугольная.')).toBeInTheDocument()
    expect(within(details!).getByText('KMB-003 не должен считаться простой PRM-009, потому что здесь должен быть основной круглый ствол.')).toBeInTheDocument()
    expect(within(details!).getByText('Sчистовая = Sствол_круглый + Sветка_прямоугольная − Sотверстие_ствол')).toBeInTheDocument()
    expect(within(details!).getByText('Sотверстие_ствол = площадь отверстия в круглом стволе под прямоугольную ветку')).toBeInTheDocument()
    expect(within(details!).getByText('Не подставлять Sотверстие_ствол = W2 × H2 как проверенный факт без CAMduct.')).toBeInTheDocument()
    expect(within(details!).getByText('Если KMB-003 использует KMB-002 как компонент: Sчистовая = Sствол_круглый + S_KMB-002 − Sотверстие_ствол')).toBeInTheDocument()
    expect(within(details!).getByText('Не ставить “Проверена” только из-за СПВА.')).toBeInTheDocument()
    expect(within(details!).queryByText(/^Проверена$/)).not.toBeInTheDocument()
    expect(within(details!).getByText('Альтернативные источники / справочные формулы')).toBeInTheDocument()
    expect(within(details!).getByText('S = [(π × D × Д1 + 2(Ш + Г) × Д2 − Ш × Г + 2(Ш + Г) × 0.05) × 1.2] × n')).toBeInTheDocument()
    expect(within(details!).getByText(/В более понятной записи: S = \[\(π × D1 × L1 \+ 2\(W2 \+ H2\) × L2 − W2 × H2 \+ 2\(W2 \+ H2\) × 0\.05\) × 1\.2\] × n/)).toBeInTheDocument()

    const rectangularRoundTakeoffRow = screen.getByText('KMB-004').closest('tr')
    expect(rectangularRoundTakeoffRow).not.toBeNull()
    fireEvent.click(rectangularRoundTakeoffRow!)

    expect(rectangularRoundTakeoffRow).toHaveAttribute('aria-selected', 'true')
    expect(within(details!).getAllByText('KMB-004').length).toBeGreaterThan(0)
    expect(within(details!).getByText('Тройник прямоугольник-круг / прямоугольный воздуховод с круглой врезкой')).toBeInTheDocument()
    expect(within(details!).getByText('KMB-004 — прямоугольный воздуховод с круглой врезкой: комбинированное изделие с прямоугольным основным стволом и круглой веткой.')).toBeInTheDocument()
    expect(within(details!).getByText('Не смешивать KMB-004 с KMB-003, PRM-010, KRG-008, KRG-009, PRM-001, PRM-005 или KRG-005.')).toBeInTheDocument()
    expect(within(details!).getByText('KMB-004 не должен считаться простой KRG-008, потому что здесь есть основной прямоугольный ствол.')).toBeInTheDocument()
    expect(within(details!).getByText('Sчистовая = Sствол_прямоугольный + Sветка_круглая − Sотверстие_ствол')).toBeInTheDocument()
    expect(within(details!).getByText('Sотверстие_ствол = π × (D2 / 2)²')).toBeInTheDocument()
    expect(within(details!).getByText('Не считать вычитание круглого отверстия проверенным CAMduct-фактом без подтверждения.')).toBeInTheDocument()
    expect(within(details!).getByText('Если KMB-004 использует KRG-008 как компонент: Sчистовая = Sствол_прямоугольный + S_KRG-008 − Sотверстие_ствол')).toBeInTheDocument()
    expect(within(details!).getByText('Не ставить “Проверена” только из-за СПВА.')).toBeInTheDocument()
    expect(within(details!).queryByText(/^Проверена$/)).not.toBeInTheDocument()
    expect(within(details!).getByText('Альтернативные источники / справочные формулы')).toBeInTheDocument()

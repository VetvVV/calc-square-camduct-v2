...
describe('split page query module routing', () => {
  ...
  it('keeps round and spiral query modules openable in split', () => {
    ...
    expect(screen.getByRole('button', { name: 'Ознакомительный расчёт' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Ознакомительный доступ' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'Рабочий кабинет подключён' })).toBeInTheDocument()

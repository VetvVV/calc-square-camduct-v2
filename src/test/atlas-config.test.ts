...
describe('atlasConfig', () => {
  it('contains a round catalog grid with active and coming soon products', () => {
    ...
    expect(roundCategory?.items).toHaveLength(20)

    const activeItems = roundCategory?.items.filter((item) => item.status === 'available') ?? []
    expect(activeItems.map((item) => item.code)).toEqual(['KRG-001', 'R-sp-001'])
    ...
    expect(futureItems).toHaveLength(18)
    expect(futureItems.every((item) => item.moduleKey === undefined)).toBe(true)
    expect(futureItems.map((item) => item.code)).toContain('KRG-019')
  })

  it('exposes PRM-001 as the first active rectangular module', () => {
    ...
    const rect001 = rectangularCategory?.items.find((item) => item.code === 'PRM-001')
